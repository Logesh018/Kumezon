...
export const getFeaturedProducts = async (req, res) => {
    try {
        let featuredProducts = await redis.get("featrued_products")
        if (featuredProducts) {
            return res.json(JSON.parse(featuredproducts));
        }

        // If it is not in the redis (cache), then we will fetch it from the database
        // .lean() return a plain javascript object instead of mongodb document, it is good for performance
