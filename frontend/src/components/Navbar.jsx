...
const Navbar = () => {
	...
	const toggleRole = async () => {
		toast(
			<span>
				<p><b>⚠️This feature is available</b></p><br /> <strong>for testing purposes only⚠️</strong>
			</span>
		);
		try {
			...
			if (res.ok) {
				...
				useUserStore.setState((state) => ({
					user: { ...state.user, role: data.role }
				}));
			} else {
				console.error(data.message);
			}
		} catch (error) {
			console.error("Failed to toggle role:", error.message);
		}
	};
				...
				<div className='flex flex-wrap justify-between items-center'>
					...
					<nav className='flex flec-wrap items-center gap-4'>
					 ...
					 ease-in-out'>
						...
						{user && (
							<button
								className='bg-emerald-800 hover:bg-emerald-700 text-white py-2 px-4 rounded-md flex items-center transition duration-300 ease-in-out'
								onClick={toggleRole}
							>
